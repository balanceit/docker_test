...
DB_CONNECTION_STRING ?= "dbname=docker_test_developement user=postgres sslmode=disable"
...
docker-run-daemon:
	docker run -d -p 8000:8080 -e DB_CONNECTION_STRING="dbname=docker_test_developement user=postgres password='' sslmode=disable" -e PGHOST=${CONTAINER_PGHOST} --log-driver json-file --name  $(CONTAINER_NAME) $(IMAGE_NAME)
