...
docker-run:
	...
	docker run --rm -it -p 8000:8080 --log-driver json-file --name $(CONTAINER_NAME) $(IMAGE_NAME) -e PGPORT=$(PGPORT) -e PGHOST=$(PGHOST)

docker-run-daemon:
	docker run -d -p 8000:8080 --log-driver json-file --name  $(CONTAINER_NAME) $(IMAGE_NAME) -e PGPORT=$(PGPORT) -e PGHOST=$(PGHOST)
