...
docker-run:
	...
	docker run --rm -it -p 8000:8080 -e PGPORT=$(PGPORT) -e PGHOST=$(PGHOST) --log-driver json-file --name $(CONTAINER_NAME) $(IMAGE_NAME)

docker-run-daemon:
	docker run -d -p 8000:8080 -e PGPORT=$(PGPORT) -e PGHOST=$(PGHOST) --log-driver json-file --name  $(CONTAINER_NAME) $(IMAGE_NAME)
